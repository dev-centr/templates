...
First, read: xref:guides/tool-hierarchy.adoc[Tool hierarchy (SW development)].
...
Corepack is experimental. To enable, install Node.js, then open an Admin Terminal, navigate to your project directory, and run `corepack enable` and follow the rest of the instructions in the link above, while referring to the xref:guides/corepack-installation-helper.adoc[Corepack installation helper].
